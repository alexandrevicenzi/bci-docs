...
Verifying container images allows you to confirm their provenance, ensuring the
...
The key used to sign all SUSE Linux Base Container Images is available for
download at https://www.suse.com/support/security/keys/[SUSE Signing Keys] and
https://ftp.suse.com/pub/projects/security/keys/container-key.pem.

The key is also available in all container images in the
`/usr/share/container-keys/suse-container-key.pem` file.

== Verifying images with Cosign

To verify a container image using Cosign, use the following command:
...
The command above fetches the signing key from the SUSE servers and uses it
to verify the latest version of SUSE Linux BCI Base.

If successful, you should see a result similar to one below:
...
Enable sigstore signatures support by editing
`/etc/containers/registries.d/default.yaml` or creating a file
...
If the verification fails, an error message as follows is returned:
